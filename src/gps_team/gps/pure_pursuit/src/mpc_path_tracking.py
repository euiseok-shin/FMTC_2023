import cvxpy
import numpy as np
from math import inf, sin, cos, tan, sqrt, pi
...
class mpc_path_tracking:
    def __init__(self, receding=5, cs=np.diag([1, 1, 1]), cu=1, cst=np.diag([1, 1, 1]), cut=1, max_speed=6, max_steer=pi/4, ref_speed=4, max_acce=6, max_acce_steer=1, sample_time=0.1, wheelbase=2, iter_threshold=0.1):
        ...
        self.receding = receding # receding horizon length -> 현재 시점부터 어느 범위까지 미래를 예측할지
        ...
        self.dt = sample_time # 몇 t 단위로 미래를 예측할 지
        self.L = wheelbase
    ...
    def controller(self, car_state, ref_path, iter_num=5, debug=False):
        ...
        # if self.cur_ind == 0:
        min_dis, self.cur_ind = self.closest_point(car_state, ref_path, self.cur_ind)
        ...
        # 후진 -> 사실 후진할 일은 없어서 추후에 삭제해도 괜찮을 듯
        if u_opt_array[0, 0] < 0:
            self.gear = 1
            self.speed_flag = -1
